...
def fastrcnn_loss(class_logits, box_regression, labels, regression_targets):
    ...
    Arguments.
        class_logits : Predict class probability information, shape=[num_anchors, num_classes].
        box_regression : Predict the edge target box regression information.
        labels : information about real classes
...
class RoIHeads(torch.nn.Module):
    ...
    def assign_targets_to_proposals(self, proposals, gt_boxes, gt_labels):
        ...
        """
        Match the corresponding gt_box for each proposal and divide into positive and negative samples
        Args:
            proposals:
            gt_boxes:
            gt_labels:
    ...
    def add_gt_proposals(self, proposals, gt_boxes):
        ...
        """
        Splice gt_boxes after proposal
        Args.
            proposals: boxes predicted by rpn for each image in a batch
            gt_boxes: real target bounding boxes for each image in a batch
    ...
    def select_training_samples(self,
        ...
        """
        Divide the positive and negative samples, count the labels corresponding to gt and the bounding box regression information
    ...
    def postprocess_detections(self,
        ...
        # type: (...) -> Tuple[List[Tensor], List[Tensor], List[Tensor]]
        """
        Post-processing of the predicted data from the network includes
        (1) calculating the final bbox coordinates based on the PROPOSAL and the predicted regression parameters
        (2) Perform softmax processing on the prediction category results
        ...
        (8) Return the topk targets sorted by scores
        Args.
            class_logits: network predicts class probability information
            box_regression: the bounding box regression parameters of network prediction
            proposals: proposals output by rpn.
            image_shapes: width and height of each image before packing it into batch.
