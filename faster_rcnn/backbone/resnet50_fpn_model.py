...
def resnet50_fpn_backbone(pretrain_path="",
    ...
    Args.
        pretrain_path: pretrain weights for resnet50, defaults to empty if not used
        norm_layer: official default is FrozenBatchNorm2d, i.e. will not update the bn layer of the parameter (because if the batch_size is set very small it will lead to worse results, it is better to not use the bn layer).
                    If you have a big GPU with a big batch_size, then you can pass in the normal BatchNorm2d layer.
                    (https://github.com/facebookresearch/maskrcnn-benchmark/issues/267)
        trainable_layers: specify which layers to train.
        returned_layers: specify which layers should be returned.
        extra_blocks: layers to be added on top of the output feature layers.
